...
public class ChannelDao extends BaseDao<Channel> implements IChannelDao {
	...
	public static void initTreeNode(List<ChannelTree> cts) {
		cts.add(0,new ChannelTree(Channel.ROOT_ID,Channel.ROOT_NAME,-1,true));
	...
	public List<ChannelTree> generateTreeByParentByasync(Integer pid) {
		if(pid==null||pid==0) {
			return this.listBySql("select id,name,pId ,(select count(*)  from t_channel t1 where t1.pId=t2.id) count from t_channel t2 where (select count(*)  from t_channel t1 where t1.pId=t2.id)>0 and t2.pId is  null order  by t2.orders ", ChannelTree.class, false);
		} else {
			return this.listBySql("select id,name,pId ,(select count(*)  from t_channel t1 where t1.pId=t2.id) count from t_channel t2 where t2.pid="+pid+" order  by t2.orders", 
					ChannelTree.class,false);
		}
